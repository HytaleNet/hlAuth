...
public final class TwoFactorPages {
     ...
     */
    public static boolean open(HlAuthPlugin plugin,
                               PlayerRef playerRef,
        ...
        CustomUIPage page = switch (result.status()) {
            ...
            case MESSENGER_VERIFY -> new MessengerVerifyPage(playerRef, plugin);
            case MESSENGER_LINK -> new MessengerLinkPage(playerRef, plugin);
            default -> null;
        };
        if (page == null) {
            return false;
        ...
        } else if (page instanceof MessengerVerifyPage verify) {
            verify.refreshTexts();
        } else if (page instanceof MessengerLinkPage link) {
            link.refreshTexts();
        }
        return true;
    }

    public static boolean open(HlAuthPlugin plugin, PlayerRef playerRef, AuthService.Result result) {
        if (result == null) {
            return false;
        }
        return switch (result.status()) {
            case TOTP_VERIFY, TOTP_SETUP, TOTP_RECOVERY, MESSENGER_VERIFY, MESSENGER_LINK ->
    ...
    public static void sendRecoveryToChat(HlAuthPlugin plugin, PlayerRef playerRef) {
    ...
    public static void cancel(HlAuthPlugin plugin,
                              PlayerRef playerRef,
                              Store<EntityStore> store,
                              Ref<EntityStore> ref) {
        boolean kick = plugin.getTotpService().cancelUi(playerRef)
            || plugin.getMessengerService().cancelUi(playerRef);
        if (kick) {
    ...
    public static boolean isTwoFactorPage(@Nullable CustomUIPage page) {
        return page instanceof TotpVerifyPage
            ...
            || page instanceof MessengerVerifyPage
            || page instanceof MessengerLinkPage;
    }
}

...
public final class LimboService {
    ...
    public LimboService(HlAuthPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    /** Player was blocked (premium/offline mismatch) and cannot log in. */
    public void markAccessDenied(UUID uuid) {
    ...
    public boolean isAccessDenied(UUID uuid) {
        LimboEntry entry = limbo.get(uuid);
        return entry != null && entry.accessDenied;
    }
    ...
    private void tick() {
        ...
        for (Map.Entry<UUID, LimboEntry> mapEntry : limbo.entrySet()) {
            ...
            if (timeoutMillis > 0 && now - entry.joinTime > timeoutMillis) {
                limbo.remove(uuid);
                com.hlauth.hytale.ui.AuthUi.closeAndDisconnect(
            ...
            if (reminderMillis > 0 && now - entry.lastReminder > reminderMillis) {
                ...
                String messengerKey = plugin.getMessengerService().reminderKey(uuid);
                if (messengerKey != null) {
                    player.sendMessage(plugin.getMessages().get(messengerKey));
                    continue;
                }

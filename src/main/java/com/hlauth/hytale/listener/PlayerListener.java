...
import com.hlauth.hytale.service.LimboProtection;
import com.hlauth.hytale.service.PremiumService;
import com.hlauth.hytale.ui.AccessDeniedPage;
import com.hlauth.hytale.ui.AuthUi;
import com.hlauth.hytale.ui.LoginPage;
import com.hlauth.hytale.ui.RegisterPage;
...
public final class PlayerListener {
    ...
    public PlayerListener(HlAuthPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    private void handleReady(AuthUi.LivePlayer live) {
        ...
        PlayerAuth blockedCheck = plugin.getDataSource().getAuth(playerRef.getUsername());
        if (blockedCheck != null && blockedCheck.blocked) {
            plugin.getLimboService().removeFromLimbo(playerRef.getUuid());
            AuthUi.closeAndDisconnect(playerRef, plugin.getMessages().get("error.accountBlocked"));
            return;
        }
    ...
    private void denyAccess(PlayerRef playerRef, AccessDeniedPage.Reason reason) {
        ...
        plugin.getLimboService().markAccessDenied(playerRef.getUuid());
    ...
    private void beginAuthFlow(PlayerRef playerRef) {
        ...
        PlayerAuth auth = plugin.getDataSource().getAuth(playerRef.getUsername());
        boolean registered = auth != null;
        if (registered && MessengerService.isBound(auth)
                && auth.messengerTwoFactorEnabled && auth.messengerConfirmOnly) {
            AuthService.Result result = plugin.getAuthService().startMessengerOnlyLogin(playerRef);
            playerRef.sendMessage(result.message());
            if (!result.success()) {
                TwoFactorPages.open(plugin, playerRef, result);
            }
    ...
    private PlayerChatEvent onChat(PlayerChatEvent event) {
        ...
        if (sender != null && plugin.getLimboService().isInLimbo(sender.getUuid())) {
            event.setCancelled(true);
            if (plugin.getLimboService().isAccessDenied(sender.getUuid())) {
                return event;
            }
            ...
            if (totpKey != null) {
                ...
                String messengerKey = plugin.getMessengerService().reminderKey(sender.getUuid());
                if (messengerKey != null) {
                    sender.sendMessage(plugin.getMessages().get(messengerKey));
                } else {
    ...
    private void onDisconnect(PlayerDisconnectEvent event) {
        ...
        plugin.getMessengerService().clear(uuid);
        plugin.getAuthService().clearPendingPostAuth(uuid);

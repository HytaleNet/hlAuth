...
public final class UnregisterCommand extends AbstractPlayerCommand {

    private final AuthMePlugin plugin;
    private final RequiredArg<String> passwordArg;

    public UnregisterCommand(AuthMePlugin plugin) {
        super("unregister", "Delete your account");
        this.plugin = plugin;
        this.passwordArg = withRequiredArg("password", "Your account password", ArgTypes.STRING);
        requirePermission("hlauth.player.unregister");
    ...
    protected void execute(@Nonnull CommandContext context,
        ...
        AuthService.Result result = plugin.getAuthService()
            .unregister(playerRef, context.get(passwordArg));
        if (result.success()) {
            AuthUi.closeAndDisconnect(playerRef, result.message());
        } else {

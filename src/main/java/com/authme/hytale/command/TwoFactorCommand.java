...
public final class TwoFactorCommand extends AbstractPlayerCommand {
    ...
    public TwoFactorCommand(AuthMePlugin plugin) {
    ...
    private void sendSetupChat(PlayerRef playerRef) {
        String secret = plugin.getTotpService().pendingSecret(playerRef.getUuid());
        String otpauth = plugin.getTotpService().pendingOtpauth(playerRef.getUuid());
        if (secret != null) {
            playerRef.sendMessage(plugin.getMessages().get("2fa.setup.secretChat", "secret", secret));
        }
        if (otpauth != null) {
            playerRef.sendMessage(plugin.getMessages().get("2fa.setup.otpauth", "uri", otpauth));
        }
    }
    ...
    private static final class DisableSub extends AbstractPlayerCommand {
        ...
        DisableSub(AuthMePlugin plugin) {
    ...
    private static final class RecoverSub extends AbstractPlayerCommand {
        ...
        RecoverSub(AuthMePlugin plugin) {
